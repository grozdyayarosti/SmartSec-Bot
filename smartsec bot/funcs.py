...
class TGHelpBot(telebot.TeleBot):
    ...
    def send_start_menu(self, message):
        if message.chat.type == 'private':
            match message.text:
                case 'Вопрос по ИБ':
                    mesg = self.send_message(
                        message.chat.id,
                        "Задайте вопрос по ИБ",
                        reply_markup=self.markUpSave('empty')
                    )
                    self.register_next_step_handler(mesg, self.question_trigger)
    ...
    def question_trigger(self, message):
        ...
        response = self.processing_response(response)
